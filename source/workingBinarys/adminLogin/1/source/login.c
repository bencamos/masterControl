...
void inputRead(int sockfd) {
  if (fork() == 0) {
    ...
    while (1){
        ...
        scanf("%s", buffer);
        int flen = strlen(buffer);
        RSA_public_encrypt(flen, buffer, encrypted, rsa2, RSA_PKCS1_OAEP_PADDING);
...
RSA *encryptionInit(int sockfd) {
  ...
  setbuf(stdout, temp);
  int rc1 = PEM_write_bio_PrivateKey(pem1, pkey, NULL, NULL, 0, 0, NULL);
  setbuf(stdout, NULL);
  free(string);
